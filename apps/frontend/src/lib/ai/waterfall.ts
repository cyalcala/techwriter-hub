...
/**
 * 🚜 START V12 SIFTER: The Atomic Ingestion Entrypoint
 * Encapsulates the entire V12 workflow for dynamic re-injection.
 */
export async function startV12Sifter(data: any) {
  const { raw_title, raw_company, raw_url, raw_html } = data;
  
  console.log(`🚜 [V12_WORKER] Sifting: ${raw_title} (${raw_company})`);
  ...
  if (existing) {
    console.log(`[V12_SIFTER] Signal Dropped: Duplicate MD5 (${md5_hash})`);
    return { status: "dropped", reason: "duplicate_md5", md5_hash };
  }

  // 3. One-Pass Orchestration (AI Extraction & Sift)
  try {
    ...
    if (!extraction.isPhCompatible || extraction.tier === 4) {
      console.log(`[V12_SIFTER] Signal Dropped: Not PH Compatible or Tier 4`);
    ...
    console.log(`[V12_SIFTER] Signal Inserted: ${extraction.title} via ${extraction.metadata?.model}`);
